...
- [ ] 11.1 HMM 레짐 분류 정상 동작 (3-state: CALM/NORMAL/VOLATILE)
- [ ] 11.2 레짐별 MIN_EDGE_BPS 동적 조정 확인 (CALM:3, NORMAL:5, VOLATILE:8)
- [ ] 11.3 레짐 전이 행렬 안정성 (급변동 없는 smooth 전환)
- [ ] 11.4 XGBoost ONNX 추론 레이턴시 <1ms 확인
- [ ] 11.5 ML 시그널 vs 기존 시그널 A/B 결과 (PnL delta > 0)
- [ ] 11.6 모델 버전 관리 정상 (engine/models/ 디렉토리)
- [ ] 11.7 Feature drift 감지 작동 (분포 이상 시 자동 경고)
- [ ] 11.8 주간 재학습 배치잡 정상 실행
- [ ] 11.9 ONNX 모델 파일 크기 합리성 (<50MB)
- [ ] 11.10 ML fallback: drift/에러 시 기존 시그널로 자동 전환

## 12. DEX 통합 (8항목) — 개발팀 + 퀀트팀

- [ ] 12.1 가스비 오라클 30초 갱신 정상 (Ethereum/Solana/Polygon)
- [ ] 12.2 가스비 fallback 고정값 작동 (RPC 장애 시)
- [ ] 12.3 Uniswap V3 실시간 가격 정확도 (CEX 대비 ±0.5% 이내)
- [ ] 12.4 CEX-DEX net spread 계산 정확 (가스비+LP fee+MEV 차감 후)
- [ ] 12.5 MEV 추정치 반영 (2-5bps 경험적 수치)
- [ ] 12.6 Bridge cost 모델 정확 (체인 간 이동 비용)
- [ ] 12.7 CostCalculator DEX 경로 통합 완료 (dex_cost.py 연동)
- [ ] 12.8 CEX-DEX Shadow 1H PnL > 0
...
> PASS 기준: 178/178 (100%) 필수. FAIL 항목이 있으면 해당 팀이 수정 후 재검사.
