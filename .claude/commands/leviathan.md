...
**원칙**: `/clear`와 `/compact`는 사용자만 실행 가능한 CLI 명령어. 자동 루프 중단 없이 다음 US로 계속 진행하되, 컨텍스트 압축은 시스템 자동 압축에 의존.

| 시점 | 동작 | 비고 |
|------|------|------|
| Phase B 완료 직후 (팀 해산 후) | 자동 진행 | Phase C 즉시 시작. 컨텍스트 자동 압축 의존 |
| US 사이클 완료 (git push 후) | 자동 진행 | prd.json/SSOT.md 디스크 기록 완료 → 다음 US Phase A 즉시 시작 |
| 사용자가 수동 개입 시 | `/clear` + `/leviathan` | 토큰 효율 최적화 원할 때. 자동 루프 재개됨 |
...
> **핵심**: 자동 루프는 멈추지 않음. `/clear`·`/compact`는 사용자가 토큰 효율화를 위해 수동으로 실행하는 선택적 명령.
> prd.json과 SSOT.md에 모든 결과가 디스크 기록되므로 컨텍스트 소실 시에도 복구 가능.
>
> **실패 사례 (US-060)**: Phase C 에이전트 실행 중 자동 컨텍스트 압축 → Shadow 결과 미수집.
> **교훈**: Phase C 진행 중에는 어떤 형태의 수동 컨텍스트 조작도 금지.
