...
import { getExchangeStatus, getSymbols } from '@/lib/api';
...
export function OrderbookView() {
  ...
  // Dynamic symbol list from API (backend returns { symbols: string[], count: number })
  const { data: symbolResponse } = useApi<{ symbols: string[]; count: number }>(
    '/api/v1/symbols',
    getSymbols,
    { refreshInterval: 30000 },
  );
  const symbolList = symbolResponse?.symbols ?? [];

  const [symbol,   setSymbol]   = useState('');
  const [exchange, setExchange] = useState(FALLBACK_EXCHANGES[0]);
  const [book,     setBook]     = useState<Orderbook | null>(null);

  // Set default symbol when list loads
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (d?.symbol === symbol && d.bids && d.asks && d.bids.length > 0 && d.asks.length > 0) {
  ...
  const maxBid = book ? Math.max(...book.bids.map(b => b.size)) : 0;
  const maxAsk = book ? Math.max(...book.asks.map(a => a.size)) : 0;
