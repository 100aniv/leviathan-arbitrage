...
| §8.12 | 대시보드 실데이터 연동 + 누락 페이지 UX | |
| §8.BL | **카나리 버그 로그 요약** (상세 → `PHOENIX_BUGLOG.md`) | 72건/v2~v41 |
...
## 8.BL 카나리 버그 로그 (v2~v41) → PHOENIX_BUGLOG.md 분리

> **72개 버그, 41개 버전의 상세 이력은 `PHOENIX_BUGLOG.md`로 분리됨** (2026-04-11).
> 본 섹션은 요약만 유지.
...
| 카테고리 | 건수 | 대표 버그 | 근본 원인 |
|----------|------|-----------|-----------|
| **인프라 (Redis/DB)** | ~18 | BUG-J (Redis NoneType 78%) | 커넥션 풀 재연결 미구현 |
| **Shadow 잔재** | ~12 | BUG-30A~G, BUG-H P1/P2 | EXECUTION_MODE=shadow, dead code paths |
| **실행 파이프라인** | ~15 | Bug 26(DeduplicationGate), BUG-70(낙관적 추적) | 동시성 race condition, 조기 종료 미정리 |
| **신호/임계값** | ~10 | Bug 27(AdaptiveThreshold 팽창), BUG-66(stale 오염) | outlier 데이터가 통계 모델 오염 |
| **거래소 API** | ~8 | BUG-68(Binance WS URL), BUG-71(매도마진) | API 변경, 불완전 검증 |
| **설정/wiring** | ~9 | Bug 1~8(v2 모니터링) | 하드코딩, config 불일치 |

### 현재 상태 (v41 기준)

- **해결**: 69/72 (✅)
- **미해결**: BUG-65 (Redis PEL 누수, LOW), 나머지 2건 추후 수정 예정
- **최대 위험**: Redis 재연결 안정성 (v16에서 78% 에러 → 이후 수정, 장기 안정성 미검증)
