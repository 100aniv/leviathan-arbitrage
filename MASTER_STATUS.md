...
| 7.3h | MIN_EDGE 5bps 최적화: 5/10/30min 전부 수익 (최대 123trades 70%WR) | 완료 | `87a7a1d` |
| **7.3i** | **60min shadow: 55trades 89%WR +0.045 PnL DD≈0% — 시간대 불문 수익** | **완료** | — |
...
| **5bps** | **60min** | **55** | **55** | **89.1% (49W/6L)** | **+0.045480** | **0.000097** | **33개** |

**결론**: MIN_EDGE_BPS=**5** 최적 확정 — **모든 시간대에서 수익**
- 5bps/5min: 12 trades, 75% WR, +0.007 — 첫 수익 확인
- 5bps/10min: 18 trades, 72% WR, +0.009 — 안정적 수익 재확인
- 5bps/30min: 123 trades, 70% WR, +0.074 — 활성 시간대 (5:30AM)
- **5bps/60min: 55 trades, 89% WR, +0.045, DD≈0% — 비활성 시간대 (6AM+) 안정성 검증**
- KRW rate: 1477.5~1478.5 (dual-source 정상 작동)
- Telegram daily summary 3회 자동 전송 확인

**시장 활동 패턴 관찰**:
- 활성 시간대 (5:30AM): 246 trades/h, 70% WR
- 비활성 시간대 (6AM+): 55 trades/h, 89% WR (낮은 빈도, 높은 품질)
- 어떤 시간대든 **PnL 양수, DD≈0%**

**72h 외삽 추정 (가중 평균)**:
- 보수적 (비활성 기준): 55건/h → **3,960건/72h**
- 낙관적 (활성 기준): 246건/h → **17,712건/72h**
- LiveGate 최소 100건 대비 **최소 39배 초과**
...
| Phase 7.3h: 5bps 30min | 175 | 5% max | **5 bps** | 123 | 69.9% (86W/37L) | +0.073758 (DD=0.07%, 활성 시간대) |
| **Phase 7.3i: 5bps 60min** | **175** | **5% max** | **5 bps** | **55** | **89.1% (49W/6L)** | **+0.045480 (DD≈0%, 비활성 시간대 검증)** |
...
1. **MIN_EDGE_BPS=5 최적 확정** (Phase 7.3h-i): 모든 테스트(5/10/30/60min)에서 양수 PnL, DD≈0%
2. **72h 예상 거래 수 (5bps)**: 보수적 55건/h → **3,960건/72h** (LiveGate 100건 기준 39배 초과)
3. **1시간 연속 Shadow 수익 확인**: 55건 거래, 89.1% 승률, +0.045480 PnL, DD=0.0097%
4. **시간대별 패턴**: 활성(5:30AM) 246/h 70%WR vs 비활성(6AM+) 55/h 89%WR — 양쪽 모두 수익
...
| 날짜 | 변경자 | 내용 |
|------|--------|------|
| 2026-03-08 | lead | Phase 7.3i: 60min shadow — 55 trades, 89.1% WR, +0.045480, DD≈0% (비활성 시간대도 수익) |
| 2026-03-08 | lead | Phase 7.3h: MIN_EDGE_BPS 5bps 최적 확정 (5/10/30min 전부 수익, 최대 123trades 70%WR) |
