...
## 프론트엔드 플로우 검증 규칙 (사장님 지시)

> 모든 시나리오에는 프론트단도 함께. 사용자 관점에서 실제 프로그램 사용 플로우를 모두 재현하고 모두 검증.

1. **실제 브라우저 검증 필수**: curl만으로 PASS 불가. MCP Chrome / browser-verifier / puppeteer CLI로 실제 화면 검증
2. **사용자 플로우 전체 재현**: 로그인→대시보드→각 페이지→기능 동작→모드 전환→설정 변경
3. **스크린샷 증거**: 각 페이지 캡처하여 `.omc/state/sit3-results/screenshots/` 저장
4. **콘솔 에러 0건**: Chrome DevTools 콘솔에서 JavaScript 에러 없음 확인
5. **모바일 뷰**: 375px/768px 반응형 검증
6. **시나리오 누락 시 추가**: 플랜에 없는 사용자 플로우 발견 시 즉시 시나리오 추가
