...
class TestAdjustBehavior:
    def test_low_win_rate_increases_edge(self):
        ...
        at.adjust(win_rate=0.35, total_trades=30)
    ...
    def test_insufficient_trades_skips_adjustment(self):
        """adjust() with trades < 30 makes no change (US-201: min_trades=30)."""
        at = AdaptiveThreshold(initial_edge_bps=10.0, min_edge=2.0, max_edge=50.0)
        before = at.current_edge_bps
        at.adjust(win_rate=0.20, total_trades=5)
        assert at.current_edge_bps == before

    def test_exactly_29_trades_skips_adjustment(self):
        """adjust() with exactly 29 trades (< 30) makes no change (US-201)."""
        at = AdaptiveThreshold(initial_edge_bps=10.0, min_edge=2.0, max_edge=50.0)
        before = at.current_edge_bps
        at.adjust(win_rate=0.20, total_trades=29)
        assert at.current_edge_bps == before

    def test_exactly_30_trades_triggers_adjustment(self):
        """adjust() with exactly 30 trades (>= 30) makes adjustment (US-201)."""
        at = AdaptiveThreshold(initial_edge_bps=10.0, min_edge=2.0, max_edge=50.0)
        before = at.current_edge_bps
        at.adjust(win_rate=0.20, total_trades=30)
        assert at.current_edge_bps != before
...
class TestHourlyCycle:
    def test_multiple_adjust_calls_accumulate(self):
        ...
        at = AdaptiveThreshold(initial_edge_bps=10.0, min_edge=2.0, max_edge=50.0)
        for _ in range(5):
            at.adjust(win_rate=0.40, total_trades=30)
