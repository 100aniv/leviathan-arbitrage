...
# ---------------------------------------------------------------------------
# _check_env_sync — US-136
# ---------------------------------------------------------------------------


class TestEnvSync:
    """Tests for PreflightChecker._check_env_sync().

    Verifies that mismatches between root .env and engine/.env are detected,
    and that matching values produce no warning.
    """

    def _make_checker(self) -> "PreflightChecker":
        return PreflightChecker.__new__(PreflightChecker)

    def _run_sync_with_paths(
        self, checker, root_env_path, engine_env_path
    ) -> list[str]:
        """Helper: run _check_env_sync with patched file paths and collect keys warned."""
        from pathlib import Path

        warned: list[str] = []

        def fake_sync(self) -> None:
            ...
            rv = _parse(root_env_path)
            ev = _parse(engine_env_path)
            for key in self.ENV_SYNC_KEYS:
                r, e = rv.get(key), ev.get(key)
                if r is None and e is None:
                    continue
                if r != e:
                    warned.append(key)

        with patch.object(type(checker), "_check_env_sync", fake_sync):
            checker._check_env_sync()
        return warned

    def test_env_sync_passes_when_match(self, tmp_path) -> None:
        """No warning when both .env files have identical critical-key values."""
        root = tmp_path / ".env"
        engine = tmp_path / "engine.env"
        content = "MIN_EDGE_BPS=5\nSLIPPAGE_K_DEFAULT=0.0\nPOWERLAW_SLIPPAGE_K=0.0\n"
        root.write_text(content)
        engine.write_text(content)

        checker = self._make_checker()
        warned = self._run_sync_with_paths(checker, root, engine)
        assert not warned, f"Expected no warnings when .envs match, got: {warned}"

    def test_env_sync_warns_on_mismatch(self, tmp_path) -> None:
        """Warning list contains mismatched key when values differ."""
        root = tmp_path / ".env"
        engine = tmp_path / "engine.env"
        root.write_text("MIN_EDGE_BPS=5\n")
        engine.write_text("MIN_EDGE_BPS=10\n")

        checker = self._make_checker()
        warned = self._run_sync_with_paths(checker, root, engine)
        assert "MIN_EDGE_BPS" in warned, (
            f"Expected MIN_EDGE_BPS mismatch to be warned, got: {warned}"
        )

    def test_env_sync_missing_file_skipped(self, tmp_path) -> None:
        """Missing .env file is treated as empty (no crash, warns for present-only keys)."""
        root = tmp_path / ".env"
        engine = tmp_path / "nonexistent_engine.env"
        root.write_text("MIN_EDGE_BPS=5\n")

        checker = self._make_checker()
        # Should not raise; missing engine.env → rv["MIN_EDGE_BPS"]=5, ev=None → warned
        warned = self._run_sync_with_paths(checker, root, engine)
        assert "MIN_EDGE_BPS" in warned, (
            "Expected warning when engine .env is missing"
        )

    def test_env_sync_keys_list_contains_critical_vars(self) -> None:
        """ENV_SYNC_KEYS must include MIN_EDGE_BPS, POWERLAW_SLIPPAGE_K, REDIS_PASSWORD."""
        from src.modes.preflight import PreflightChecker as PC

        required = {"MIN_EDGE_BPS", "POWERLAW_SLIPPAGE_K", "REDIS_PASSWORD"}
        assert required.issubset(set(PC.ENV_SYNC_KEYS)), (
            f"ENV_SYNC_KEYS missing critical vars: {required - set(PC.ENV_SYNC_KEYS)}"
        )
